...
int main(int argc, char* argv[])
{
	...
	std::cout << "Queue Families found" << std::endl;
	...
	std::cout << "Got depth format " << std::endl;
	...
	std::cout << "Made depth image " << std::endl;
	...
	// Iterate through data by interleaving vertices, normals, and texture coords
	...
	// Use VMA to allocate memory for buffer:
	...
	chk(vmaCreateBuffer(allocator, &bufferCI, &vBufferAllocCI,
		&vBuffer, &vBufferAllocation, &vBufferAllocInfo));

	// Now that we have a buffer in GPU address space,
	// Send data to GPU
	memcpy(vBufferAllocInfo.pMappedData, vertices.data(), vBufSize);
	memcpy(((char*)vBufferAllocInfo.pMappedData) + vBufSize,
		indices.data(), iBufSize);

	std::cout << "Made gpuBuffer" << std::endl;
	// Make the shader buffers per frames in flight:
	for (auto i = 0; i < maxFramesInFlight; i++)
	{
		...
		chk(vmaCreateBuffer(allocator, &uBufferCI, &uBufferAllocCI,
			&shaderDataBuffers[i].buffer, &shaderDataBuffers[i].allocation,
			&shaderDataBuffers[i].allocationInfo));

		// Get a device address for shader buffers:
	...
	// Make the fences and semaphores per our frames in flight:
	...
	std::cout << "Made Command Pool " << std::endl;
	// Make as many command buffers as frames in flight
	...
	for (auto i = 0; i < textures.size(); i++)
	{
		...
		std::string filename = "assets/suzanne" + std::to_string(i)
			+ ".ktx";
		...
		VmaAllocationCreateInfo texImageAllocCI{
			.usage = VMA_MEMORY_USAGE_AUTO };
		chk(vmaCreateImage(allocator, &texImgCI,
			&texImageAllocCI, &textures[i].image, &textures[i].allocation,
			nullptr));
		...
		// Upload data to gpu so that it can store texels in image object:
		...
		// Copy data over:
		memcpy(imgSrcAllocInfo.pMappedData,
			ktxTexture->pData, ktxTexture->dataSize);

		// Copy image data from buffer to gpu, create a command buffer
		// Also a fence to wait for it to finish
		...
		std::cout << "Made commandBuffers" << std::endl;

		// Record commands to get image data to dest:
	...
	.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT
		...
		// Clean up and store descripters
	...
	// Create descripter set layout to talk to device abt textures:
	...
	chk(vkCreateDescriptorSetLayout(device, &descLayoutTexCI,
		nullptr, &descriptorSetLayoutTex));
...
.descriptorCount = static_cast<uint32_t>(textures.size())
	...
	// Where we are going to write descriptor sets:
...
slangSession->loadModuleFromSource("triangle", "assets/shader.slang", nullptr, nullptr)
	...
	// Create a shader module with code:
	...
	// Create pipeline layout for graphics pipeline:
	...
	// Vertex attributes that match cpu-side vertex struct
	...
	// Pipeline state for vertex input
	...
	// Shader pipeline stages. It's only 1 due to slang but will need to change for more
	...
	// Viewport and scissor done dynamically
	...
	// Depth stencil state for depth buffering:
	...
	// Dynamic rendering instead of render passes probably will change later :(
	...
	// Set blending, rasterization, and multisampling to sane values:
	...
	//Lets make this graphics pipeline:
	...
	while (!quit)
	{
		...
		for (SDL_Event event; SDL_PollEvent(&event);)
		{
			...
			// Window resize
			// Window resize (also handled by resizingEventWatcher during active drags,
